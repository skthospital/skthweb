function SwopPopupCtrl($rootScope,$scope,$modalInstance,$http,$compile,$timeout,$location,$log,$modal,$window,params_mem_first,params_mem_second,work,jet) {
    $log.info('Enter SwopPopupCtrl');

    $scope.firstMember = params_mem_first
    $scope.secondMember = params_mem_second
    ...
    $scope.cancel = function () {
        ...
        $scope.criteria.day_first = {}
        $scope.criteria.day_second = {}
        angular.forEach($rootScope.criteria.personalList,function (item){
          item.icon = "fa-calendar-o"
      		item.colorIcon = "blue"
        })
        $rootScope.criteria.nameTrade.length = 0
    };
}
