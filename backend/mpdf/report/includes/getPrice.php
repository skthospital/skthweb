...
function getPrice ($user_id,$ward_id,$month_id,$year_id) {
  ...
  if($conn){
    ...
    $sql = "SELECT * FROM SKTH_ENS.dbo.tblScheduleDetail
    INNER JOIN SKTH_HOSPITAL.dbo.tblPersonal p on p.Mem_ID = s.schd_user_id
    INNER JOIN SKTH_HOSPITAL.dbo.tbl_Position o on o.Position_ID = p.Position_ID
    WHERE s.schd_month = '$month_id' AND s.schd_year = '$year_id' AND s.schd_ward_id = '$ward_id' AND s.schd_user_id = '$user_id' ";
